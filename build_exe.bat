@echo off
REM ================================================
REM  Whisper PTT GUI v0.8.4 – Setup + .exe Build
REM ================================================
setlocal
echo.
echo  ================================================
echo   Whisper PTT GUI v0.8.4 – Setup und .exe Build
echo  ================================================
...
REM OpenVINO fuer Intel NPU Unterstuetzung (Intel Core Ultra AI Boost)
pip install openvino openvino-genai openvino-tokenizers --quiet

echo.
echo [4/5] CUDA-Check...
python -c "import sys; import torch; g=torch.cuda.get_device_name(0) if torch.cuda.is_available() else 'Keine'; print(f'  GPU: {g}')" 2>nul || echo   GPU: Keine GPU (NPU oder CPU wird verwendet)
